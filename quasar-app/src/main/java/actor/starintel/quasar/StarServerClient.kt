...
internal data class Endpoint(val id: String, val method: String, val path: String, val legacy: Boolean)
internal data class LoginResult(val apiKey: String, val username: String, val mustChangePassword: Boolean)
...
internal class StarServerClient(context: Context) {
    ...
    private fun request(method: String, path: String, body: Any? = null): JSONObject {
        ...
        check(server.isNotBlank() && !token.isNullOrBlank()) { "Configure a Star server first" }
        return requestAt(server, token, method, path, body)
    }

    private fun requestAt(server: String, token: String?, method: String, path: String, body: Any? = null): JSONObject {
        ...
        return try {
            ...
            connection.setRequestProperty("User-Agent", "quasar-android/${BuildConfig.VERSION_NAME}")
            if (body != null) {
    ...
    private fun normalizeServer(value: String): String {
        ...
        require(server.startsWith("https://") || (BuildConfig.DEBUG && server.startsWith("http://"))) {
...
internal fun parseLoginResponse(root: JSONObject): LoginResult {
    val apiKey = root.optString("api_key")
    require(apiKey.startsWith("star_sk_v1_") && apiKey.length <= 4096) { "Login response did not contain a valid API key" }
...
internal class StarHttpFailure(val status: Int, detail: String) :
    RuntimeException("HTTP $status${detail.takeIf { it.isNotBlank() }?.let { " · $it" }.orEmpty()}")

internal fun resultDocument(row: JSONObject): JSONObject = row.optJSONObject("doc") ?: row

internal fun resultId(row: JSONObject): String {
