...
class ActivityGraphPreferences(private val context: android.content.Context) {
    ...
    fun range(): ActivityRange = runCatching {
        ActivityRange.valueOf(
            prefs.getString(KEY_RANGE, ActivityRange.H24.name) ?: ActivityRange.H24.name,
        )
    }.getOrDefault(ActivityRange.H24)

    fun setRange(range: ActivityRange) {
        prefs.edit().putString(KEY_RANGE, range.name).apply()
    }

    companion object {
        private const val PREFS = "starintel_graph"
...
class ActivityGraphComplicationService : SuspendingComplicationDataSourceService() {
    override suspend fun onComplicationRequest(request: ComplicationRequest): ComplicationData? {
        ...
        // Reuse the normal repository/cache cadence. Only a successful network
        // refresh records a new cumulative total; cached reads do not invent a
        // second history observation.
        StarIntelRepository.get(applicationContext).snapshot()

        val range = ActivityGraphPreferences(applicationContext).range()
        val points = ActivityHistoryStore.get(applicationContext).points(range)
        val images = ActivityGraphRenderer.render(points, range)
    ...
    override fun getPreviewData(type: ComplicationType): ComplicationData? {
        ...
        val images = ActivityGraphRenderer.renderPreview(range)
        return graphData(images, "StarIntel activity preview ${range.label}")
    }

    private fun graphData(
        images: ActivityGraphImages,
        description: String,
    ): SmallImageComplicationData {
        val smallImage = SmallImage.Builder(
            Icon.createWithBitmap(images.active),
            SmallImageType.ICON,
...
data class ActivityGraphImages(
    val active: Bitmap,
    val ambient: Bitmap,
)
...
object ActivityGraphRenderer {
    ...
    fun render(
        ...
        range: ActivityRange,
    ): ActivityGraphImages = ActivityGraphImages(
        active = draw(points, range, ambient = false, preview = false),
        ambient = draw(points, range, ambient = true, preview = false),
    )
    ...
    fun renderPreview(range: ActivityRange): ActivityGraphImages {
        ...
        val points = values.mapIndexed { index, value ->
            ActivityPoint(
                epochSeconds = now - (values.lastIndex - index) * 300L,
                documentsAdded = value,
            )
        }
        return ActivityGraphImages(
            active = draw(points, range, ambient = false, preview = true),
            ambient = draw(points, range, ambient = true, preview = true),
        )
    }
    ...
    private fun draw(
        ...
        points.forEachIndexed { index, point ->
            ...
            val y = PAD_TOP + graphHeight * (1f - normalized.coerceIn(0f, 1f))
            val px = previousX
            val py = previousY
