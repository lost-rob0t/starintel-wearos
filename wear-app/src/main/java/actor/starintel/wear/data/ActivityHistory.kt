...
internal object ActivityHistoryModel {
    ...
    fun record(state: ActivityHistoryState, sample: ActivitySample): ActivityHistoryState {
        ...
        val hourlyWithoutBucket = state.hourly.filter { it.epochSeconds / 3600L != hour }
        val hourly = (hourlyWithoutBucket + sample)
            .sortedBy { it.epochSeconds }
            .filter { it.epochSeconds >= sample.epochSeconds - LONG_RETENTION_SECONDS }
            .takeLast(MAX_HOURLY_SAMPLES)

        return ActivityHistoryState(detail = detail, hourly = hourly)
    }
    ...
    fun points(
        ...
        val minTime = nowSeconds - range.seconds
        val samples = source.filter { it.epochSeconds in minTime..nowSeconds }
        if (samples.size < 2) return emptyList()

        val gapLimit = if (range.seconds <= ActivityRange.H24.seconds) {
            DETAIL_GAP_SECONDS
        } else {
            HOURLY_GAP_SECONDS
        }
        ...
        return samples.zipWithNext { previous, current ->
            ...
            val delta = if (!reset && !gap) current.documentsTotal - previous.documentsTotal else null
            ActivityPoint(
                epochSeconds = current.epochSeconds,
                documentsAdded = delta,
                gap = gap,
                reset = reset,
            )
        }
    ...
    private fun upsertSorted(
        samples: List<ActivitySample>,
        sample: ActivitySample,
    ): List<ActivitySample> = (samples.filter { it.epochSeconds != sample.epochSeconds } + sample)
        .sortedBy { it.epochSeconds }
}

class ActivityHistoryStore private constructor(context: Context) {
    private val prefs = context.applicationContext.getSharedPreferences(PREFS, Context.MODE_PRIVATE)

    @Synchronized
    fun record(snapshot: StarIntelSnapshot) {
        if (!snapshot.reachable || snapshot.receivedAt <= 0L) return
        val epochSeconds = snapshot.generatedAt.takeIf { it > 0L } ?: snapshot.receivedAt / 1000L
        val next = ActivityHistoryModel.record(
            load(),
            ActivitySample(epochSeconds, snapshot.documentsTotal),
        )
        save(next)
    }
    ...
    ): List<ActivityPoint> = ActivityHistoryModel.points(load(), range, nowSeconds)
    ...
    private fun load(): ActivityHistoryState = runCatching {
        ...
        ActivityHistoryState(
            detail = decode(root.optJSONArray("detail")),
            hourly = decode(root.optJSONArray("hourly")),
        )
    ...
    private fun encode(samples: List<ActivitySample>): JSONArray = JSONArray().also { array ->
        samples.forEach { sample ->
            array.put(JSONArray().put(sample.epochSeconds).put(sample.documentsTotal))
        }
    }
    ...
    companion object {
        ...
        @Volatile
        private var instance: ActivityHistoryStore? = null
