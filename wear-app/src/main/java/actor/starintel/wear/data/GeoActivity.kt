...
private const val GEO_MAX_RESPONSE_BYTES = 256 * 1_024

data class GeoCoordinate(val latitude: Double, val longitude: Double)
data class GeoBucket(val latitude: Double, val longitude: Double, val count: Int)
...
internal object GeoActivityModel {
    fun parseSearchDocuments(raw: String): List<GeoCoordinate> {
        val rows = JSONObject(raw).optJSONArray("rows") ?: return emptyList()
        return buildList {
    ...
    fun aggregate(coordinates: List<GeoCoordinate>, maxBuckets: Int = GEO_MAX_BUCKETS): List<GeoBucket> {
        if (coordinates.isEmpty() || maxBuckets <= 0) return emptyList()
        return coordinates.groupingBy { coordinate ->
            val latCell = floor((coordinate.latitude + 90.0) / GEO_BUCKET_DEGREES).toInt().coerceIn(0, 11)
            val lonCell = floor((normalizeLongitude(coordinate.longitude) + 180.0) / GEO_BUCKET_DEGREES).toInt().coerceIn(0, 23)
            latCell to lonCell
        }.eachCount().map { (cell, count) ->
            GeoBucket(
                -90.0 + (cell.first + 0.5) * GEO_BUCKET_DEGREES,
                -180.0 + (cell.second + 0.5) * GEO_BUCKET_DEGREES,
                count,
            )
        }.sortedWith(compareByDescending<GeoBucket> { it.count }.thenBy { it.latitude }.thenBy { it.longitude })
            .take(maxBuckets)
    }

    private fun parseCoordinate(doc: JSONObject): GeoCoordinate? {
        val data = doc.optJSONObject("data")
        val latitude = number(data, "lat") ?: number(data, "latitude") ?: number(doc, "lat") ?: number(doc, "latitude")
        val longitude = number(data, "long") ?: number(data, "lon") ?: number(data, "longitude") ?: number(doc, "long") ?: number(doc, "lon") ?: number(doc, "longitude")
        if (latitude == null || longitude == null || !latitude.isFinite() || !longitude.isFinite()) return null
        if (latitude !in -90.0..90.0 || longitude !in -180.0..180.0) return null
        return GeoCoordinate(latitude, normalizeLongitude(longitude))
    }
    ...
    private fun normalizeLongitude(value: Double): Double = when {
        value == 180.0 -> -180.0
        value < -180.0 || value > 180.0 -> ((value + 180.0) % 360.0 + 360.0) % 360.0 - 180.0
        else -> value
    }
}
...
class GeoActivityRepository private constructor(context: Context) {
    ...
    suspend fun snapshot(forceRefresh: Boolean = false): GeoActivitySnapshot = withContext(Dispatchers.IO) {
        ...
        if (base.isBlank() || apiKey.isNullOrBlank()) {
            return@withContext GeoActivitySnapshot(false, false, error = "StarIntel setup required")
        }

        val cached = loadCached(now, base, apiKey)
        if (!forceRefresh && cached != null && now - cached.fetchedAtMs < GEO_CACHE_MS) return@withContext cached

        runCatching {
            val stats = starIntelRepository.snapshot(false)
            if (!stats.reachable && stats.receivedAt <= 0L) error(stats.error ?: "StarIntel stats unavailable")
            val total = (stats.documentsByType["geo"] ?: 0L) + (stats.documentsByType["address"] ?: 0L)
            ...
            ).also { saveCached(it, base, apiKey) }
        }.getOrElse { failure ->
            cached?.copy(reachable = false, stale = true, error = failure.message ?: failure.javaClass.simpleName)
                ?: GeoActivitySnapshot(true, false, stale = true, error = failure.message ?: failure.javaClass.simpleName)
        }
    }

    fun clear() = prefs.edit().remove(KEY_JSON).apply()

    private fun search(base: String, apiKey: String, query: String): String {
        val encoded = URLEncoder.encode(query, Charsets.UTF_8.name())
        val connection = URL("$base/api/v1/search?q=$encoded&limit=$GEO_SEARCH_LIMIT_PER_TYPE").openConnection() as HttpURLConnection
        return try {
            ...
            if (connection.contentLengthLong > GEO_MAX_RESPONSE_BYTES) error("geo search response too large")
            connection.inputStream.bufferedReader().use { reader ->
                ...
                while (true) {
                    ...
                    if (read < 0) break
                    total += read
                    if (total > GEO_MAX_RESPONSE_BYTES) error("geo search response too large")
    ...
    private fun saveCached(snapshot: GeoActivitySnapshot, base: String, apiKey: String) {
        ...
        snapshot.buckets.forEach { bucket -> buckets.put(JSONArray().put(bucket.latitude).put(bucket.longitude).put(bucket.count)) }
        val root = JSONObject()
            .put("version", 1)
    ...
    private fun loadCached(now: Long, base: String, apiKey: String): GeoActivitySnapshot? = runCatching {
        ...
        if (root.optInt("version", 0) != 1 || root.optString("scope") != scopeFingerprint(base, apiKey)) return null
    ...
    private fun scopeFingerprint(base: String, apiKey: String): String =
        MessageDigest.getInstance("SHA-256").digest("$base\u0000$apiKey".toByteArray(Charsets.UTF_8))
            .joinToString("") { "%02x".format(it) }

    companion object {
        private const val PREFS = "starintel_geo_activity"
        private const val KEY_JSON = "geo_v1"
        @Volatile private var instance: GeoActivityRepository? = null
